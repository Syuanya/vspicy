...
@Service
public class AdminSystemDictService {
    ...
    public AdminSystemDictService(SysDictTypeMapper dictTypeMapper,
    ...
    public SystemDictOverviewView overview() {
        ...
        return new SystemDictOverviewView(
                safe(typeCount),
    ...
    public List<SystemDictTypeView> listTypes(String keyword, Integer status, Integer limit) {
    ...
    public SystemDictTypeView getType(Long id) {
        return toTypeView(mustGetType(id));
    }

    @Transactional
    public SystemDictTypeView createType(SystemDictTypeCommand command) {
    ...
    @Transactional
    public SystemDictTypeView updateType(Long id, SystemDictTypeCommand command) {
    ...
    @Transactional
    public SystemDictTypeView enableType(Long id) {
        SysDictType type = mustGetType(id);
        type.setStatus(1);
        dictTypeMapper.updateById(type);
        return toTypeView(type);
    ...
    @Transactional
    public SystemDictTypeView disableType(Long id) {
        SysDictType type = mustGetType(id);
        type.setStatus(0);
        dictTypeMapper.updateById(type);
        return toTypeView(type);
    ...
    public List<SystemDictItemView> listItems(String typeCode, String keyword, Integer status, Integer limit) {
    ...
    public SystemDictItemView getItem(Long id) {
        return toItemView(mustGetItem(id));
    }

    @Transactional
    public SystemDictItemView createItem(SystemDictItemCommand command) {
    ...
    @Transactional
    public SystemDictItemView updateItem(Long id, SystemDictItemCommand command) {
    ...
    @Transactional
    public SystemDictItemView enableItem(Long id) {
        SysDictItem item = mustGetItem(id);
        item.setStatus(1);
        dictItemMapper.updateById(item);
        return toItemView(item);
    ...
    @Transactional
    public SystemDictItemView disableItem(Long id) {
        SysDictItem item = mustGetItem(id);
        item.setStatus(0);
        dictItemMapper.updateById(item);
        return toItemView(item);
    ...
    @Transactional
    public SystemDictItemView moveItem(Long id, SystemDictItemMoveCommand command) {
        SysDictItem item = mustGetItem(id);
        ensureEditable(item.getEditable(), "该字典项为系统内置，不允许移动");
        if (command == null || command.sortNo() == null) {
            throw new BizException("排序值不能为空");
        }
        item.setSortNo(command.sortNo());
        dictItemMapper.updateById(item);
        return toItemView(mustGetItem(id));
    }
    ...
    private void validateType(SystemDictTypeCommand command, boolean create) {
        if (command == null) {
    ...
    private void validateItem(SystemDictItemCommand command, boolean create) {
        if (command == null) {
    ...
    private void applyType(SysDictType type, SystemDictTypeCommand command, boolean create) {
        if (create || command.typeCode() != null) {
    ...
    private void applyItem(SysDictItem item, SystemDictItemCommand command, boolean create) {
        if (create || command.typeCode() != null) {
    ...
    private SystemDictTypeView toTypeView(SysDictType type) {
        if (type == null) {
            return null;
        }
        Long itemCount = dictItemMapper.selectCount(new LambdaQueryWrapper<SysDictItem>()
                .eq(SysDictItem::getTypeCode, type.getTypeCode()));
        return new SystemDictTypeView(
                type.getId(),
                type.getTypeCode(),
                type.getTypeName(),
                type.getDescription(),
    ...
    private SystemDictItemView toItemView(SysDictItem item) {
        if (item == null) {
            return null;
        }
        return new SystemDictItemView(
                item.getId(),
                item.getTypeCode(),
                item.getItemLabel(),
                item.getItemValue(),
